...
- Reddit/X 社交弱信号默认关闭，启用时使用独立配置和官方 API/授权方式；帖子不能直接入报，必须先回链可核验的原始出处。
